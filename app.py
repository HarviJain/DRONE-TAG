...
#working for multiple tracker data fetching and displaying not for senor and not for guest mode(before guest mode)
...
@app.route('/api/sensor/dashboard', methods=['POST'])
@login_required
def fetch_sensor_dashboard():
    """
    Fetch live sensor data from external API for the dashboard sensor panel
    """
    try:
        data = request.get_json()
        sensor_id = data.get("sensor_id")
        ...
        # Format sensor ID (3 digits)
        sensor_id_str = str(sensor_id).zfill(3)
        
        print(f"Fetching sensor data for ID: {sensor_id_str}")
        ...
        if isinstance(api_data, dict):
            if "points" in api_data:
                api_points = api_data.get("points", [])
            else:
                # If the API returns data directly in dict format
                api_points = [api_data]
        ...
        for point in api_points:
            ...
            processed_point = {
                "SensorId": point.get("SensorId", sensor_id_str),
                ...
                "Moisture": float(point.get("Moisture", 0)) if point.get("Moisture") else 0,
                "Temperature": float(point.get("Temperature", 0)) if point.get("Temperature") else 0,
                "EC": float(point.get("EC", 0)) if point.get("EC") else 0,
                "PHValue": float(point.get("PHValue", 0)) if point.get("PHValue") else 0,
                "Nitrogen": float(point.get("Nitrogen", 0)) if point.get("Nitrogen") else 0,
                "Phosphorous": float(point.get("Phosphorous", 0)) if point.get("Phosphorous") else 0,
                "Potassium": float(point.get("Potassium", 0)) if point.get("Potassium") else 0,
                "SatelliteFix": point.get("SatelliteFix", "N/A")
            }
            processed_points.append(processed_point)

        # Sort by timestamp (newest first)
        try:
            processed_points.sort(key=lambda x: x.get("Timestamp", ""), reverse=True)
        except Exception:
            pass
        ...
        logging.exception("Sensor dashboard API failed")
        return jsonify({"error": str(e)}), 500
