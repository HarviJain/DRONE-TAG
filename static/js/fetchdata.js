...
// Guest trackers that should be hidden from regular users
const GUEST_TRACKERS = ['3001', '3002', '3003', '3004', '3005'];

// Function to check if current user is guest (from Flask template)
function isGuestUser() {
    // Check if there's a meta tag or data attribute indicating guest mode
    // This would need to be set by the Flask template
    return document.body.dataset.userType === 'guest';
}

// Function to filter out guest trackers
function filterGuestTrackers(trackers) {
    if (isGuestUser()) {
        return trackers; // Guest users can see guest trackers
    }
    return trackers.filter(id => !GUEST_TRACKERS.includes(id));
}

// Clean up guest trackers from localStorage for regular users
function cleanupGuestTrackers() {
    if (!isGuestUser()) {
        // Clean up saved trackers
        const savedTrackers = JSON.parse(localStorage.getItem('saved_trackers')) || [];
        const filteredTrackers = savedTrackers.filter(id => !GUEST_TRACKERS.includes(id));
        localStorage.setItem('saved_trackers', JSON.stringify(filteredTrackers));
        
        // Clean up group settings
        const groupSettings = JSON.parse(localStorage.getItem('groupSettings')) || {};
        const cleanedGroupSettings = {};
        
        Object.keys(groupSettings).forEach(groupName => {
            // Skip the guest group entirely
            if (groupName.toLowerCase() === 'guest') {
                return;
            }
            
            // Filter out guest trackers from other groups
            if (Array.isArray(groupSettings[groupName])) {
                cleanedGroupSettings[groupName] = groupSettings[groupName].filter(id => !GUEST_TRACKERS.includes(id));
            } else {
                // Handle object format (with visibility and color)
                const cleanedTrackers = {};
                Object.keys(groupSettings[groupName]).forEach(trackerId => {
                    if (!GUEST_TRACKERS.includes(trackerId)) {
                        cleanedTrackers[trackerId] = groupSettings[groupName][trackerId];
                    }
                });
                if (Object.keys(cleanedTrackers).length > 0) {
                    cleanedGroupSettings[groupName] = cleanedTrackers;
                }
            }
        });
        
        localStorage.setItem('groupSettings', JSON.stringify(cleanedGroupSettings));
        
        // Clean up real-time parameters
        const realtimeSettings = JSON.parse(localStorage.getItem('realtime_parameters')) || {};
        const cleanedRealtimeSettings = {};
        Object.keys(realtimeSettings).forEach(trackerId => {
            if (!GUEST_TRACKERS.includes(trackerId)) {
                cleanedRealtimeSettings[trackerId] = realtimeSettings[trackerId];
            }
        });
        localStorage.setItem('realtime_parameters', JSON.stringify(cleanedRealtimeSettings));
        
        // Clean up auto-update trackers
        const autoUpdateTrackers = JSON.parse(localStorage.getItem('auto_update_trackers')) || [];
        const filteredAutoUpdate = autoUpdateTrackers.filter(id => !GUEST_TRACKERS.includes(id));
        localStorage.setItem('auto_update_trackers', JSON.stringify(filteredAutoUpdate));
    }
}
...
document.addEventListener('DOMContentLoaded', function () {
  ...
  function updateLegend() {
    ...
    // Add trackers to legend (filter out guest trackers for non-guest users)
    Object.entries(trackerColorMap).forEach(([id, color]) => {
      if (isTrackerVisible(id)) {
        // Skip guest trackers for non-guest users
        if (!isGuestUser() && GUEST_TRACKERS.includes(id)) {
          return;
        }
        html += `
          <div style="display:flex;align-items:center;gap:6px;margin-bottom:2px;">
  ...
  fetchBtn?.addEventListener('click', () => {
    const id = trackerInput.value.trim();
    if (id) {
      // Prevent fetching guest trackers for non-guest users
      if (!isGuestUser() && GUEST_TRACKERS.includes(id)) {
        showStatus('Guest trackers are not available for regular users', 'error');
        return;
      }
      fetchSingleTracker(id, true);
    }
  });
  ...
  async function fetchSingleTracker(trackerId, clearBefore = false, enableRealtime = false) {
    ...
    // Prevent fetching guest trackers for non-guest users
    if (!isGuestUser() && GUEST_TRACKERS.includes(trackerId)) {
      showStatus('Guest trackers are not available for regular users', 'error');
      return;
    }
  ...
  window.fetchGroupTrackers = function (trackerIds, enableRealtime = false) {
    ...
    // Filter out guest trackers for non-guest users
    let filteredTrackerIds = trackerIds;
    if (!isGuestUser()) {
      filteredTrackerIds = trackerIds.filter(id => !GUEST_TRACKERS.includes(id));
    }
    
    const visibleTrackers = filteredTrackerIds.filter(id => isTrackerVisible(id));
  ...
  // Run cleanup to remove guest trackers from localStorage for regular users
  cleanupGuestTrackers();
  ...
  document.addEventListener('keydown', function(e) {
    ...
    if (e.altKey && e.key === 'a') {
      ...
      const trackerId = trackerInput.value.trim();
      if (trackerId) {
        // Prevent enabling real-time for guest trackers
        if (!isGuestUser() && GUEST_TRACKERS.includes(trackerId)) {
          showStatus('Guest trackers are not available for regular users', 'error');
          return;
        }
  ...
  window.addEventListener('load', function() {
    ...
    // Filter out guest trackers for regular users
    const filteredTrackers = filterGuestTrackers(savedTrackers);
    
    const autoUpdateEnabled = JSON.parse(localStorage.getItem('auto_update_enabled')) || false;
    const autoUpdateTrackers = JSON.parse(localStorage.getItem('auto_update_trackers')) || [];
    
    if (filteredTrackers.length > 0) {
      showStatus(`Loading ${filteredTrackers.length} saved trackers...`, 'loading');
      
      setTimeout(() => {
        filteredTrackers.forEach((trackerId, index) => {
          setTimeout(() => {
            const enableRealtime = autoUpdateEnabled && autoUpdateTrackers.includes(trackerId);
            fetchSingleTracker(trackerId, index === 0, enableRealtime);
          }, index * 1000);
