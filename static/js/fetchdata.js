...
function updateSensorTable(sensorData = null) {
  ...
  // Define the CORRECT order we want columns to appear
  const columnOrder = [
    'SensorId', 'Timestamp', 
    'Moisture', 'Temperature', 'EC', 'PHValue',  // Sensor parameters first
    'Nitrogen', 'Phosphorous', 'Potassium',       // Nutrients
    'Latitude', 'Longitude', 'Altitude',          // Location (after Potassium)
    'SatelliteFix', 'Maplink'                     // Additional info
  ];
  ...
  // Filter to only include keys that are in our column order AND exist in the data
  ...
  headers.forEach(h => {
    const th = document.createElement("th");
    th.textContent = formatSensorHeader(h);
    // Add coordinate column class for toggling
    if (['Latitude', 'Longitude', 'SatelliteFix'].includes(h)) {
      th.className = 'coord-column';
    }
    trHead.appendChild(th);
  });
  thead.appendChild(trHead);
