...
// Guest trackers for guest mode
const GUEST_TRACKERS = ['3001', '3002', '3003', '3004', '3005'];

// Function to initialize guest group
function initializeGuestGroup() {
    const groups = JSON.parse(localStorage.getItem('tracker_groups')) || {};
    
    // Create or update guest group
    if (!groups['guest']) {
        groups['guest'] = GUEST_TRACKERS;
        localStorage.setItem('tracker_groups', JSON.stringify(groups));
    }
    
    // Add guest trackers to saved trackers
    const savedTrackers = JSON.parse(localStorage.getItem('saved_trackers')) || [];
    let updated = false;
    
    GUEST_TRACKERS.forEach(trackerId => {
        if (!savedTrackers.includes(trackerId)) {
            savedTrackers.push(trackerId);
            updated = true;
        }
    });
    
    if (updated) {
        localStorage.setItem('saved_trackers', JSON.stringify(savedTrackers));
    }
    
    // Initialize group settings for guest group
    const groupSettings = JSON.parse(localStorage.getItem('groupSettings')) || {};
    if (!groupSettings['guest']) {
        groupSettings['guest'] = {};
        const defaultColors = ['#FF6B6B', '#4ECDC4', '#FFD166', '#06D6A0', '#118AB2'];
        
        GUEST_TRACKERS.forEach((trackerId, index) => {
            groupSettings['guest'][trackerId] = {
                visible: true,
